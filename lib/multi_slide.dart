...
import 'package:multi_slide/line_chart.dart';

class MultiSlide extends StatefulWidget {
  final Size size;

  MultiSlide(this.size);

  @override
  _MultiSlideState createState() => _MultiSlideState(size);
}

enum SlideDirection { Left, Right, Up, Down }

class _MultiSlideState extends State<MultiSlide> {
  final Size size;
  final double miniPageWidth;
  final double quarter;

//  _MultiSlideState(this.size, this.miniPageWidth, this.quarter);

  _MultiSlideState(this.size)
      : miniPageWidth = size.width / 4 * 3,
        quarter = size.width / 4;

  List<String> titles = ['最新', '涨幅', '涨跌', '涨速', '大单净量', '总手', '换手', '量比'];
  ...
  @override
  void initState() {
    rightController = ScrollController(initialScrollOffset: quarter * 3);
  ...
  handleEnd(DragEndDetails details) {
    if (slideDirection == SlideDirection.Left ||
      ...
      if (!rightAnimated &&
          rightController.offset != 0 &&
          rightController.offset < quarter * 3) {
        if ((quarter * 3 - rightController.offset) > quarter / 2 &&
  ...
  handleUpdate(DragUpdateDetails details) {
    if ((details.globalPosition.dx - lastPos.dx).abs() >
        (details.globalPosition.dy - lastPos.dy).abs()) {
      ///横向滑动
    ...
    switch (slideDirection) {
      ...
      case SlideDirection.Right:
        ...
        } else if (rightController.offset != 0 &&
            rightController.offset <= quarter * 3) {
      ...
      case SlideDirection.Up:
        if (stockVerticalController.offset <
            stockVerticalController.position.maxScrollExtent) {
      ...
      case SlideDirection.Down:
        if (stockVerticalController.offset >
            stockVerticalController.position.minScrollExtent) {
          stockVerticalController.jumpTo(stockVerticalController.offset - disV);
          stockNameController.jumpTo(stockNameController.offset - disV);
          chartController.jumpTo(stockNameController.offset - disV);
        }
  ...
  Widget buildBottomPart(Size size) {
    return SingleChildScrollView(
      ...
      child: Container(
        ...
        width: quarter * 4 + titles.length * quarter,
        height: size.height,
        child: Row(
          children: [
            ///left
